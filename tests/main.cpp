...
int main(int argc, char* argv[])
{
    ...
    // if tests to run were specified on cmd line
    if (argc > 1) {
        std::string testName;
        // build a set to eliminate possible name duplication
        for (int i = 1; i < argc; ++i) {
            testName = argv[i];
            if (std::string::npos == testName.find("::") && std::string::npos == testName.find("TestSuite")) {
                testName += "TestSuite";
            }
            testsToRun.insert(testName);
        }
    } else {
        // Magic string which indicates that all tests should be executed.
        testsToRun.insert("All Tests");
    }

    std::set<std::string>::const_iterator iter = testsToRun.begin();
    for (; iter != testsToRun.end(); ++iter) {
